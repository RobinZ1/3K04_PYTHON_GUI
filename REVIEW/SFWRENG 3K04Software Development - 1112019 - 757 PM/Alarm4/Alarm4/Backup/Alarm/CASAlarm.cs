...
namespace Alarm
{
    ...
    public class CASAlarm
    {
        ...
        private enum internalVariable                           //possible internal variables
        {
        ...
        /**PUBLIC************************************************/
        /* returns max number of code digits                                          */
        /********************************************************/
        public int getMaxCodeDigits()
        {
            return maxCodeDigits;
        }
         ...
         * appropriate internalVariable and submits that to
        ...
        public void setMonitoredVariable(monitoredVariable m)
        {
            switch (m)
            {
                case monitoredVariable.m_one:
                    if (checkDigit('1'))
                        nextState(internalVariable.goodDigit);
                    else
                        nextState(internalVariable.badDigit);
                    break;
                ...
                case monitoredVariable.m_clear:
                    nextState(internalVariable.clearButton);
                    break;
                case monitoredVariable.m_set:
                    nextState(internalVariable.setButton);
                    break;
                case monitoredVariable.m_trip:
                    nextState(internalVariable.trip);
                    break;
            }
        }

        /*------------------------------------------------------*/
        /* Uses the internal variable i to determine the next      *
         * operating state of the device.                       */
        /*------------------------------------------------------*/
        private void nextState(internalVariable i)
        {
            switch (opState)
            {
                case alarmState.deviceOff:
                    if (i == internalVariable.setButton)
                        opState = alarmState.deviceOn;
                    break;
                case alarmState.deviceOn:
                    if (i == internalVariable.badDigit)
                        opState = alarmState.error;
                    else if (i == internalVariable.goodDigit)
                        opState = alarmState.good1;
                    else if (i == internalVariable.trip)
                        opState = alarmState.alarmOn;
                    break;
                case alarmState.error:
                    if (i == internalVariable.clearButton)
                        opState = alarmState.deviceOn;
                    else if (i == internalVariable.trip)
                ...
                case alarmState.good1:
                    if (i == internalVariable.badDigit)
                        opState = alarmState.error;
                    else if (i == internalVariable.clearButton)
                        opState = alarmState.deviceOn;
                    else if (i == internalVariable.goodDigit)
